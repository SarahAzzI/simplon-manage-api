...
class InscriptionService:
    ...
    def create(db: Session, data: InscriptionCreate) -> Inscription:
        ...
        if user.role != Role.STUDENT:
            raise BadRequestException(
                f"L'utilisateur {data.user_id} n'est pas un apprenant."
            )
    ...
    def get_by_student(
        ...
        return db.scalars(
            select(Inscription)
            .options(
                joinedload(Inscription.session).joinedload(SessionFormation.formation)
            )
    ...
    @staticmethod
    def count_by_student(db: Session, user_id: int) -> int:
        return (
            db.scalar(
                select(func.count(Inscription.id)).where(Inscription.user_id == user_id)
            )
            or 0
        )
    ...
    @staticmethod
    def update(
        db: Session, inscription_id: int, data: InscriptionUpdate
    ) -> Inscription:
        inscription = db.get(Inscription, inscription_id)
        if not inscription:
            raise NotFoundException("Inscription", inscription_id)
